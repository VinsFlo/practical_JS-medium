			...
			var modals = function modals() {
					...
					trigger.forEach(function (item) {
						item.addEventListener('click', function (e) {
							...
							windows.forEach(function (item) {
								...
								item.classList.add('animated', 'fadeIn'); //анимация
							});
					...
					setTimeout(function () {
						...
						document.querySelectorAll('[data-modal]').forEach(function (item) {
							if (getComputedStyle(item).display !== 'none') {
								//Если у нас модальное окно, которое сейчас перебирается будет показано пользователю, то ...
								display = "block";
							}
						});
				...
				function openByScroll(selector) {
					window.addEventListener('scroll', function () {
						var scrollHeight = Math.max(document.documentElement.scrollHeight, document.body.scrollHeight); //под старые браузеры

						if (!btnPressed && window.pageYOffset + document.documentElement.clientHeight >= scrollHeight) {
							//Сравниваем сумму с полной высотой страницы, так мы вычисляемЮ долистал ли пользователь до конца
...
/* harmony import */ var core_js_modules_web_dom_collections_for_each__WEBPACK_IMPORTED_MODULE_0___default = /*#__PURE__*/__webpack_require__.n(core_js_modules_web_dom_collections_for_each__WEBPACK_IMPORTED_MODULE_0__);
			...
			var sliders = function sliders(slides, dir, prev, next) {
				...
				try {
					//если селекторы кнопок не были переданы, это не ломает всю логику скрипта
